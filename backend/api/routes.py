...
async def generate_pdf(
    ...
    try:
        ...
        recommendations = generate_all_recommendations(
            skill_validation_results=data.skill_validation_details.model_dump(),
            grammar_results=get_default_grammar_results(),
            location_results=get_default_location_results(),
            score_results={"formatting_score": data.component_scores.formatting},
            sections={},  
            keyword_analysis=data.jd_match_analysis.model_dump() if data.jd_match_analysis else None,
            resume_keywords=data.skills,
        )

        # Step 2: Format and summarize
        formatted_recs = format_recommendations_for_api(recommendations)
        summary_text   = get_recommendation_summary(recommendations)

        # Step 3: Build full report dictionary
        report_data = {
            "ats_score": data.ATS_score,
            "summary_text": summary_text,
            "component_scores": data.component_scores.model_dump(),
        ...
        # Step 4: Generate PDF
        pdf_bytes = generate_combined_pdf(report_data)
...
async def generate_history_pdf(
    ...
    try:
        # Build recommendations from stored analysis
        recommendations = generate_all_recommendations(
            skill_validation_results=analysis_data.get("skill_validation_details", {}),
            grammar_results=get_default_grammar_results(),
            location_results=get_default_location_results(),
            score_results=analysis_data.get("component_scores", {}),
            sections=analysis_data.get("sections", {}),
            keyword_analysis=analysis_data.get("jd_comparison"),
            resume_keywords=analysis_data.get("skills", []),
        )

        html_docs = format_recommendations_for_api(recommendations)
